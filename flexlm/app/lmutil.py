...
#                dt = datetime.strptime(start, timeformat).replace(year=thisyear, microsecond=0, second=0)
    ...
    def get_fp():
        ...
        if Config.TEST_MODE:
            ...
            #print("Running", Config.LMSTAT)
            p = subprocess.Popen(Config.LMSTAT, stdout=subprocess.PIPE, bufsize=1)
            fp = p.stdout
    ...
    @staticmethod
    def read():
        fp = ReadLmstat.get_fp()
        data_dict = ReadLmstat.parse_lmstat(fp)
        fp.close()
        return data_dict
